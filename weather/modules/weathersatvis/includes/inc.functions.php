...
{
  $CheckTopicArray = lingx_DBArray("select sat_image_id from ".DB_TABLE_SATVIS_IMAGES." where sat_image_name='".lingx_Security($ImageName)."' limit 1", true);
  if($CheckTopicArray['Num'] == 0)
  {
	$Date = date('Y-m-d H:i:s');
	lingx_DBQuery("insert into ".DB_TABLE_SATVIS_IMAGES." (sat_image_date, sat_image_name) values ('".lingx_Security($Date)."', '".lingx_Security($ImageName)."')");
  }
}
...
function lingx_GetRemoteSatvisImages($FTPArray, $LocalDir, $RemoteDir, $RemoteFiles, $DownloadInterval)
{
  ...
  if(@!file_exists($LocalDir.'index.php'))
  {
   ...
   {
	  $GetLastDateQuery = lingx_DBQuery("select sat_image_date from ".DB_TABLE_SATVIS_IMAGES." order by sat_image_date desc limit 1");
...
function lingx_ClearCache($LocalDir, $ClearCacheInterval)
{
  ...
  $GetImagesQuery = lingx_DBQuery("select sat_image_id, sat_image_name from ".DB_TABLE_SATVIS_IMAGES." where sat_image_date<'".lingx_Security($GetImagesDate)."'");
  while($CacheImageArray = lingx_DBFetchArray($GetImagesQuery))
  {
	...
	lingx_DBQuery("delete from ".DB_TABLE_SATVIS_IMAGES." where sat_image_id='".lingx_Security($CacheImageArray['sat_image_id'])."'");
	$NumDeleted++;
  }
  return $NumDeleted;
}
